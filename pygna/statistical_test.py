...
class StatisticalTest:
    ...
    def get_null_distribution_mp(self, geneset,iter=100,n_proc=1):
        ...
        if n_proc==1:
            ...
            results = [p.apply_async(StatisticalTest.get_null_distribution, args=(self,geneset,n_trial)) for w in list(range(1,n_proc+1))]
...
def geneset_hitting_time_statistic(network, geneset, diz={}, observed_flag=False):
    ...
    try:
        ...
        print("The hitting time dictionary must be passed as the value of a dictionary with key hitting_time")
        raise

    nodes=diz["hitting_time"]["nodes"]

    hitting_time=[diz["hitting_time"]["tau"][nodes.index(i),nodes.index(j)] for i in geneset for j in geneset if i!=j]
