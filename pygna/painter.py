...
class Painter_RW(Painter):
    ...
    def plot_matrix(self, show_labels=False, conversion_table = None):
        ...
        if show_labels:
            if not conversion_table:
                logging.warning('Pass a conversion table to plot the annotation')
            else:
                tab_conversion = pd.read_table(tab_conversion,
                sep="\t",
                )
                labels = []
