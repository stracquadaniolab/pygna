...
class Painter_RW(Painter):
    ...
    def plot_matrix (self, show_labels=False):
        ...
        logging.info(str(np.array(self.geneset)[:,np.newaxis].shape )+str(self.RW_matrix.shape))

        

        if show_labels:
            tab_conversion=pd.read_table('/home/viola/Desktop/geneset-network-analysis/primary_data/entrez_name.tsv', sep="\t" )
            labels=[]
            for i in self.geneset:
                name=tab_conversion[tab_conversion["EntrezID"]==int(i)]["Symbol"].values.tolist()
                if len(name)>0:
                    labels.append(str(name[0]))
                else:
                    labels.append(i)

            g2=sns.heatmap(self.RW_matrix-mask,cmap="OrRd",square=True,ax=axes,xticklabels=labels,yticklabels=labels,cbar=True,vmin=0, vmax=np.max(self.RW_matrix-mask))
        ...
        fig.savefig(self.output_folder+"RW_matrix_"+self.name+".pdf", format="pdf")

    def restrict_network(self, geneset):

        self.__network = nx.DiGraph(self.__network.subgraph(max(nx.connected_components(network), key=len)) )

    def add_annotation_nodes_in_geneset (self, geneset, annotation="in_subset"):

        logging.info("adding annotation to nodes in geneset")
        logging.info("%d genes in geneset,%d mapped in network ", (len(geneset),len(self.__universe.intersection(set(geneset)))) )
    ...
    def draw_graphml(self):
        ...
        nx.write_graphml(self.__network, self.__output_folder+self.__name+".graphml")

def paint_datasets_stats(table_filename, output_folder, stat_name ,alternative='greater'):
    ...
    if alternative=="greater":
        for i,row in table.sort_values(by=["empirical_pvalue","observed"],ascending=[True, False]).iterrows():
            data=data.append({"setname": row["setname"],
        ...
        g = sns.catplot(x="value", y="setname", hue="type", data=data,
                        height=6, kind="bar", palette=palette_binary, orient="h" ,aspect=1 )
        ...
        for i,r in data[data["type"]=="observed"].iterrows():#.sort_values(by=["pvalue"],ascending=True)
            if r["pvalue"]==0:
                pval="<%1.1E" %(1./n_permutations)
            else:
                pval="=%.4f" %r["pvalue"]
            g.facet_axis(0,0).annotate( "pval:"+pval,xy=(r["value"],k), xytext=(r["value"],k) ,color=col[r["pvalue"]<(0.1*2/len(data))],fontsize=6)
            k+=1
        ...
        for i,row in table.sort_values(by=["empirical_pvalue","observed"],ascending=[True, True]).iterrows():
            data=data.append({"setname": row["setname"],
                                "pvalue":row["empirical_pvalue"],
                                "type": "observed",
                                "value":row["observed"],
        ...
        for i,r in data[data["type"]=="observed"].iterrows():#.sort_values(by=["pvalue"],ascending=True)
            if r["pvalue"]==0:
                ...
                pval="=%.4f" %r["pvalue"]
            g.facet_axis(0,0).annotate( "pval:"+pval,xy=(r["value"],k), xytext=(r["value"]+0.05,k-0.10) ,color=col[r["pvalue"]<(0.1*2/len(data))],fontsize=6)
            k+=1
...
def paint_comparison_stats(table_filename, output_folder, stat_name):
    ...
    annot=table.pivot(values="empirical_pvalue", index="setname_A",columns="setname_B").values
    ...
    g2=sns.heatmap(pivot_table.T,cmap=palette,ax=axes,square=True,xticklabels=1,yticklabels=1,
                    cbar=True,center=0, linewidths=.1,linecolor="white")#,annot=annot,fmt=""
...
def paint_comparison_RW(table_filename, output_folder, stat_name):
    ...
    g2=sns.heatmap(pivot_table.T,cmap=palette,ax=axes,square=True,xticklabels=1,yticklabels=1,mask=mask,
                    cbar=True, linewidths=.1,linecolor="white")#,annot=annot,fmt=""#,mask=mask,
...
def paint_final_table (final_table, output_file):
    ...
    out.apply_multiple_testing_correction(final_table, pval_col='empirical_pvalue', method='fdr_bh', threshold=0.05)

    with open(final_table) as f:
        table=pd.read_csv(f)
    
    table['log']=-np.log10(table['empirical_pvalue'].values)
    table['log']=table['log'].replace(to_replace=np.inf, value= np.max(table[table['log']!=np.inf]['log'])+0.5)

    setnames=list(set(table.setname.values.tolist())) 

    f,ax= plt.subplots(1, figsize=(8,20))
    g= sns.barplot(x="log", y="setname", hue='analysis',data=table, palette=sns.color_palette("muted", len(setnames)) )
    ax.axvline(x=1.3, ymin=0, ymax=len(setnames), alpha=0.5,color='k', linestyle='--', linewidth=0.5)
    ax.axvline(x=4, ymin=0, ymax=len(setnames), alpha=0.5,color='k', linestyle='--', linewidth=0.5)
    ...
    for a, tab in table.groupby(['analysis']):
        ...
        print(tab.head())
        diz[a]['ranking']=[(i) for i in zip(tab.index.values.tolist(), tab['setname'].values.tolist())]
        diz[a]['tab']=tab
    ...
    for i in range(len(analysis)):
        for j in range(len(analysis)):
            s_corr,s_pvalue=stats.spearmanr(diz[analysis[i]]['tab']['rank'].values,diz[analysis[j]]['tab']['rank'].values, axis=0, nan_policy='propagate')
            print('%s, %s : spearman r--- corr: %f, p %f' %(analysis[i], analysis[j],s_corr, s_pvalue))
            M_corr[i,j]=s_corr
