...
def plot_null_distribution(null_distribution, observed, output_folder, setname=None):
    ...
    if len(null_distribution[null_distribution > observed]):
        g3= sns.distplot(null_distribution[null_distribution > observed], hist=False, kde=False, rug=True, color= "r", ax=axes)
    g4= axes.stem([observed], [0.01], "r","r--" )

    sns.despine(ax=axes,top=True, bottom=False, right=True, left=True)
    axes.annotate( "observed=%d"%observed,xy=(observed,0.01),xytext=(observed-45,0.01) ,color="r",fontsize=10)
    #plt.show()

    logging.info("output file for figure: "+output_folder+setname+"_null_distribution.pdf")
    fig.savefig(output_folder+setname+"_null_distribution.pdf", format= "pdf")




def draw_graph(network: "network file",
                output_file: "name graphml network for visualisation"):
    '''
        Plotting graph
    '''
